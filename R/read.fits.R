read.fits = function(file, hdu = 0, comments = TRUE, strip = c(" ","'"," "), maxlines = 50000, xlo = NA, xhi = NA, ylo = NA, yhi = NA){
    ...
    if(!is.na(xlo)){
        if(!is.na(wcspix[1])){
            ...
            hdr = set.fitskey(key="CRPIX1", value=wcspix[1], hdr=hdr)
        }
    }
    if(!is.na(ylo)){
        if(!is.na(wcspix[2])){
            wcspix[2] = wcspix[2] - ylo + 1
            hdr = set.fitskey(key="CRPIX2", value=wcspix[2], hdr=hdr)
        }
    }
    ...
    while(xhdu & thdu>nhdu){
        ...
        if(xhdu){
            ...
            if(length(grep("BINTABLE",hdr[which(hdr[,"key"]=="XTENSION"),"value"]))>0){
                ...
                xhdu = FALSE # BINTABLE is last FITS file in extension(?)
            }else{
                du = .read.fits.image(fcon, hdr, xlo, xhi, ylo, yhi)
            }
...
.parse.fits.hdr = function(rawhead, comments=TRUE, strip=c(" ","'"," ")){
    ...
    for(i in 1:(nchar(rawhead)/80)){
        ...
        if(k=="COMMENT" | k=="HISTORY"){
            ...
            while(((length(grep("'",strsplit(poss[1],"")[[1]]))%%2) != 0) & (length(poss)>1)){
        ...
        if(comments){
            if(k=="COMMENT" | k=="HISTORY"){
                ...
                m = substr(row,9,80) # no stripping of comments/history
            }else{
                # any comments?
                if(length(poss) == 1){
                    m = ""
